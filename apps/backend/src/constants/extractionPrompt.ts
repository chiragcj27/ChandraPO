...
PER-ITEM ACCURACY (CRITICAL — each item must reflect only that row's data):
- One document row = one item in the "items" array. Each item must be filled ONLY from the cells of that specific row.
- Do NOT copy a value from one row into another. Do NOT carry over VendorStyleCode, OrderQty, Description, or any field from the previous or next row. Every field (except ItemPoNo, which is the same for all) must come from the current row only.
- For tables: (1) Identify the header row and note which column is which (e.g. Style, Article, Qty, Description). (2) For each data row, read the cell under each header. (3) Use the client mapping to know which client column maps to VendorStyleCode, OrderQty, etc. (4) Fill the item using only that row's cell values.
- ItemPoNo: use the PO/Order number from the document header for every item (same value for all). All other fields are per-row and must be read from that row.
- VendorStyleCode, OrderQty, ItemSize, ItemRefNo, and instruction/remarks fields must be the exact values visible in that row (or ""/null if not present). Do not infer or reuse from another row.
- For enum fields (Metal, Tone, Category, StockType, MakeType): extract from that row's Description/Category/text only; do not copy from adjacent rows.

General guidance:
- For PDF files: Identify the buyer/client name from the PO header (not Chandra Jewels). Extract the PO/Order/Invoice number from the header as invoice_number. Extract the PO/Order date if present; else leave "". Locate the item table: find the header row (column names), then process every data row in order. Items are presented with serial numbers; extract every serial-numbered row. The count of items MUST match the serial-numbered rows, with no missing or extra items.
...
- CRITICAL: For ALL enum fields (Metal, Tone, StockType, MakeType), the values MUST be extracted primarily from the Description field of THAT ROW. These enum values are almost always embedded within item descriptions. For each row, parse only that row's description/category text to identify metal, tone, stock type, make type; match to the exact enum options provided above. Do not use another row's description.
- Preserve the serial order of rows in the items array.
- Do not hallucinate or infer extra items; only output what exists. Do not invent or duplicate field values across rows.
