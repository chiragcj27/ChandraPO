...
const mapExtractionLineToRecord = (line: ExtractedLine): POItemRecord => ({
  isIncomplete: line.isIncomplete ?? true,
  ...
  productionDeliveryDate: normalizeDate(line.deadlineDate ?? undefined) ?? null,
  expectedDeliveryDate: normalizeDate(line.shippingDate ?? undefined) ?? null,
...
const buildPOFromExtraction = async (
  ...
  const itemRecords = (extraction.items || []).map((line) => mapExtractionLineToRecord(line));
  ...
  const poNumber = extraction.po?.poNumber?.trim() || generatePoNumberFromFilename(fileMeta.filename);
  const poDate = extraction.po?.poDate ? normalizeDate(extraction.po.poDate) ?? new Date() : new Date();
  ...
  return {
    ...
    clientName: clientNameOverride || extraction.po?.clientName || 'Unknown Client',
    totalItems: extraction.po?.totalItems ?? itemRecords.length,
    incompleteItems: extraction.po?.incompleteItems ?? itemRecords.filter((item) => item.isIncomplete).length,
    totalValue: Number(extraction.po?.totalValue ?? 0) || 0,
    status: 'New',
...
export const uploadPO = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    console.log(`[Backend][upload] step=1_request file=${file.originalname} | PO upload request received, size=${(file.size / 1024).toFixed(2)} KB`);
    ...
    // For large files (>10MB), use S3 URL approach to avoid Render timeout
    // For smaller files, we can still use the direct upload approach
    const fileSizeMB = file.size / (1024 * 1024);
    const useS3Url = fileSizeMB > 10; // Use S3 URL for files larger than 10MB
    
    let extraction: ExtractedPOResponse;
    
    if (useS3Url) {
      console.log(`[Backend][upload] step=2_s3_first file=${file.originalname} | Large file (${fileSizeMB.toFixed(2)} MB), uploading to S3 first to avoid timeout`);
      
      // Step 1: Upload to S3 first
      console.log(`[Backend][upload] step=2a_s3_start file=${file.originalname} | Uploading file to S3...`);
      const ur = await s3Service.uploadFile(file.buffer, file.originalname, file.mimetype, 'pos');
      uploadResult = ur;
      const fm = await upsertFileMetadata({
        key: ur.key,
        path: ur.url,
        filename: file.originalname,
      });
      fileMeta = fm;
      console.log(`[Backend][upload] step=2a_s3_done file=${file.originalname} | S3 upload done, key=${ur.key}`);
      
      // Step 2: Generate signed download URL
      console.log(`[Backend][upload] step=2b_signed_url file=${file.originalname} | Generating signed download URL...`);
      const signedUrl = await s3Service.getSignedDownloadUrl(ur.key, ur.bucket, 600); // 10 minutes expiry
      console.log(`[Backend][upload] step=2b_signed_url_done file=${file.originalname} | Signed URL generated`);
      
      // Step 3: Call FastAPI with S3 URL (avoids Render timeout)
      console.log(`[Backend][upload] step=2c_extract_start file=${file.originalname} | Calling FastAPI with S3 URL (client=${selectedClientName ?? 'none'}, expectedItems=${expectedItems ?? 'none'})`);
      extraction = await fastapiService.extractPurchaseOrderFromS3(signedUrl, file.originalname, {
        clientName: selectedClientName ?? undefined,
        mappingText: mappingText ?? undefined,
        expectedItems,
      });
      console.log(`[Backend][upload] step=2c_extract_done file=${file.originalname} | FastAPI extraction complete`);
    } else {
      // For smaller files, use parallel approach (original method)
      console.log(`[Backend][upload] step=2_parallel file=${file.originalname} | Small file (${fileSizeMB.toFixed(2)} MB), using parallel S3 upload and FastAPI extraction`);
      
      const [s3Result, extractionResult] = await Promise.all([
        (async () => {
          console.log(`[Backend][upload] step=2a_s3_start file=${file.originalname} | Uploading file to S3...`);
          const ur = await s3Service.uploadFile(file.buffer, file.originalname, file.mimetype, 'pos');
          uploadResult = ur;
          ...
          fileMeta = fm;
          console.log(`[Backend][upload] step=2a_s3_done file=${file.originalname} | S3 upload done, key=${ur.key}`);
          return { uploadResult: ur, fileMeta: fm };
        })(),
        (async () => {
          console.log(`[Backend][upload] step=2b_extract_start file=${file.originalname} | Calling FastAPI (client=${selectedClientName ?? 'none'}, expectedItems=${expectedItems ?? 'none'})`);
          return fastapiService.extractPurchaseOrder(file, {
            clientName: selectedClientName ?? undefined,
            mappingText: mappingText ?? undefined,
            expectedItems,
          });
      ...
      uploadResult = s3Result.uploadResult;
      fileMeta = s3Result.fileMeta;
      extraction = extractionResult;
      console.log(`[Backend][upload] step=3_both_done file=${file.originalname} | S3 and FastAPI both complete`);
    }

    const poPayload = await buildPOFromExtraction(extraction, fileMeta, selectedClientName ?? undefined);
    console.log(`[Backend][upload] step=4_build file=${file.originalname} | Extraction received, building PO payload, poNumber=${poPayload.poNumber}, items=${poPayload.items?.length ?? extraction?.items?.length ?? 0}`);
    ...
    console.log(`[Backend][upload] step=5_save file=${file.originalname} | PO saved to DB, poNumber=${existing.poNumber}`);

    console.log(`[Backend][upload] step=6_done file=${file.originalname} | Responding 201 to client`);
    res.status(201).json({
      message: 'PO uploaded and processed successfully',
      po: mapDocToDTO(existing.toObject()),
    });
  } catch (error) {
    console.error(`[Backend][upload] error file=${(req.file as Express.Multer.File)?.originalname ?? 'unknown'} |`, error);
